const createYourOwnButton = document.querySelector(
    ".create-button"
) as HTMLButtonElement | null; // selecting create button
const welcomeSection = document.querySelector(
    ".welcome"
) as HTMLDivElement | null; //selecting welcome section
const cvFormSection = document.querySelector(
    ".create-cv-section"
) as HTMLDivElement | null;
const addSkillsButton = document.querySelector(
    ".add-skills-button"
) as HTMLButtonElement | null;
const resume = document.querySelector(".resume") as HTMLDivElement | null;
let downloadCVButton = document.querySelector(".cv-download-button") as HTMLButtonElement | null;
...
const feedCV = (
    ...
    let userNameDiv = document.querySelector(".user-name") as HTMLHeadElement;
    let userNameDiv_2 = document.querySelector(".name") as HTMLSpanElement;
    let emailDiv = document.querySelector(".email") as HTMLSpanElement;
    let phoneNumber = document.querySelector(".phonenumber") as HTMLSpanElement;
    let address = document.querySelector(".address") as HTMLSpanElement;
    let headlineField = document.querySelector(".headline") as HTMLHeadingElement;
    let summaryField = document.querySelector(".summary") as HTMLParagraphElement;
    let profileImageField = document.querySelector(
        ".user-profile"
    ) as HTMLImageElement;

    let educationSection = document.querySelector(
        ".section-3"
    ...
    let skillsSection = document.querySelector(
        ".skills-list"
    ) as HTMLUListElement;
    ...
    if (imageFile) {
        ...
        reader.onload = function (e: any) {
            if (e.target && e.target.result) {
                profileImageField.src = e.target.result as string;
            }
        };
...
//_________types for objects required for storing data for education and experience
interface Education {
...
const form = document.querySelector(
    ".create-cv-form"
) as HTMLFormElement | null;  //_________selecting form_________
...
form?.addEventListener("submit", function handleForm(e) {
    ...
    let email = document.getElementsByName("userEmail")[0] as HTMLInputElement;
    let phoneNumber = document.getElementsByName(
        "phoneNumber"
    )[0] as HTMLInputElement;
    let profileHeadline = document.getElementsByName(
        "profileHeadline"
    )[0] as HTMLTextAreaElement;
    let professionalSummary = document.getElementsByName(
        "professionalSummary"
    )[0] as HTMLTextAreaElement;
    let linkedIn = document.getElementsByName("linkedIn")[0] as HTMLInputElement;
    let address = document.getElementsByName("address")[0] as HTMLInputElement;

    let educationTitle_1 = document.getElementsByName(
        "educationTitle"
    )[0] as HTMLInputElement;
    let educationTimelines_1 = document.getElementsByName(
        "educationTimeline"
    )[0] as HTMLInputElement;
    let educationDescription_1 = document.getElementsByName(
        "educationDescription"
    )[0] as HTMLTextAreaElement;

    let educationTitle_2 = document.getElementsByName(
        "educationTitle"
    )[1] as HTMLInputElement;
    let educationTimelines_2 = document.getElementsByName(
        "educationTimeline"
    )[1] as HTMLInputElement;
    let educationDescription_2 = document.getElementsByName(
        "educationDescription"
    )[1] as HTMLTextAreaElement;

    let jobTitle_1 = document.getElementsByName(
        "jobTitle"
    )[0] as HTMLInputElement;
    let jobTimelines_1 = document.getElementsByName(
        "jobTimeline"
    )[0] as HTMLInputElement;
    let jobDescription_1 = document.getElementsByName(
        "jobDescription"
    )[0] as HTMLTextAreaElement;

    let jobTitle_2 = document.getElementsByName(
        "jobTitle"
    )[1] as HTMLInputElement;
    let jobTimelines_2 = document.getElementsByName(
        "jobTimeline"
    )[1] as HTMLInputElement;
    let jobDescription_2 = document.getElementsByName(
        "jobDescription"
    )[1] as HTMLTextAreaElement;

    let skills = document.querySelectorAll("#skill");

    let profileImage = document.getElementsByName(
        "person"
    )[0] as HTMLInputElement;

    const education_1: Education = {
    ...
    if (cvFormSection && resume && skills.length >= 5) {
        ...
        feedCV(
            ...
            profileImage.files ? profileImage.files[0] : "" // __________validate check for images
        );
    } else {
